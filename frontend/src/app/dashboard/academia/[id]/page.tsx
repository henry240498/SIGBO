...
const COLOR_ESTADO_INSCRIPCION: Record<string, string> = {
  INSCRITO: 'var(--neutral-fill)',
  ACTIVO: 'var(--info-fill)',
  RETIRADO: 'var(--bad-fill)',
  FINALIZADO: 'var(--ok-fill)',
};
...
export default function DetalleActividadAcademicaPage() {
  ...
  if (error && !actividad) return <p style={{ color: 'var(--danger)' }}>{error}</p>;
  if (!actividad) return <Cargando texto="Cargando actividad…" />;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div className="card">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            <h2 style={{ fontSize: 18 }}>
              {actividad.nombre}
              {actividad.codigo && <span style={{ color: 'var(--muted)', fontSize: 14 }}> ({actividad.codigo})</span>}
            </h2>
            <div style={{ display: 'flex', gap: 8, marginTop: 6, flexWrap: 'wrap' }}>
              ...
              {actividad.esExterna && <span className="badge" style={{ background: 'var(--neutral-fill)' }}>externa</span>}
              {puedeEditar ? (
                <select
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 16, marginTop: 16 }}>
          ...
          <div>
            <span style={{ fontSize: 11, color: 'var(--muted)', display: 'block' }}>Lugar</span>
            <span style={{ fontSize: 13 }}>{actividad.lugar ?? '-'}</span>
          </div>
          <div>
            <span style={{ fontSize: 11, color: 'var(--muted)', display: 'block' }}>Carga horaria</span>
            <span style={{ fontSize: 13 }}>{actividad.duracionHoras ? `${actividad.duracionHoras} hs` : '-'}</span>
          </div>
        </div>
        {actividad.descripcion && (
          <p style={{ fontSize: 13, color: 'var(--muted)', marginTop: 12 }}>{actividad.descripcion}</p>
        )}
      </div>
            ...
            <form onSubmit={agregarExternoForm} style={{ display: 'flex', flexDirection: 'column', gap: 10, borderTop: '1px solid var(--line)', paddingTop: 10 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
                ...
                <div>
                  ...
                  <input id="documento" className="input-field" value={externo.documento} onChange={(e) => setExterno({ ...externo, documento: e.target.value })} />
                </div>
                <div>
                  <label htmlFor="institucion" style={{ fontSize: 12, display: 'block', marginBottom: 4 }}>Institución</label>
                  <input id="institucion" className="input-field" value={externo.institucion} onChange={(e) => setExterno({ ...externo, institucion: e.target.value })} />
                </div>
                <div>
                  <label htmlFor="especialidad" style={{ fontSize: 12, display: 'block', marginBottom: 4 }}>Especialidad</label>
                  <input id="especialidad" className="input-field" value={externo.especialidad} onChange={(e) => setExterno({ ...externo, especialidad: e.target.value })} />
                </div>
                <div>
                  <label htmlFor="telefono" style={{ fontSize: 12, display: 'block', marginBottom: 4 }}>Teléfono</label>
                  <input id="telefono" className="input-field" value={externo.telefono} onChange={(e) => setExterno({ ...externo, telefono: e.target.value })} />
                </div>
              </div>
      ...
      <div className="card">
        ...
        {instructores && instructores.length === 0 && <p style={{ color: 'var(--muted)', fontSize: 13 }}>Sin instructores asignados.</p>}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ textAlign: 'left', borderBottom: '1px solid var(--line)' }}>
                ...
                <th scope="col" style={{ padding: '6px 4px' }}>Detalle</th>
                {puedeGestionarInstructores && <th scope="col" style={{ padding: '6px 4px' }}>Acciones</th>}
              </tr>
            </thead>
            <tbody>
              {instructores.map((i) => (
        ...
        <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {mostrarFormParticipanteExterno && (
            <form
              ...
              style={{ display: 'flex', flexDirection: 'column', gap: 10, borderTop: '1px solid var(--line)', paddingTop: 10 }}
            >
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
                <div>
                  <label htmlFor="nombre-2" style={{ fontSize: 12, display: 'block', marginBottom: 4 }}>Nombre</label>
                  <input id="nombre-2"
                    className="input-field"
                ...
                <div>
                  <label htmlFor="apellido-2" style={{ fontSize: 12, display: 'block', marginBottom: 4 }}>Apellido</label>
                  <input id="apellido-2"
                    className="input-field"
                ...
                <div>
                  <label htmlFor="cedula" style={{ fontSize: 12, display: 'block', marginBottom: 4 }}>Cédula</label>
                  <input id="cedula"
                    className="input-field"
      ...
      <div className="card">
        ...
        {participantes && participantes.length === 0 && <p style={{ color: 'var(--muted)', fontSize: 13 }}>Sin participantes inscritos.</p>}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ textAlign: 'left', borderBottom: '1px solid var(--line)' }}>
                <th scope="col" style={{ padding: '6px 4px' }}>Nombre</th>
                <th scope="col" style={{ padding: '6px 4px' }}>Tipo</th>
                <th scope="col" style={{ padding: '6px 4px' }}>Inscripción</th>
                <th scope="col" style={{ padding: '6px 4px' }}>Estado</th>
                <th scope="col" style={{ padding: '6px 4px' }}>Resultado final</th>
                <th scope="col" style={{ padding: '6px 4px' }}>Costo</th>
                {puedeInscribir && <th scope="col" style={{ padding: '6px 4px' }}>Acciones</th>}
              </tr>
            </thead>
            <tbody>
              {participantes.map((p) => (
                <tr key={p.id} style={{ borderBottom: '1px solid var(--line-soft)' }}>
                  ...
                  <td style={{ padding: '6px 4px', color: 'var(--muted)' }}>{p.fechaInscripcion}</td>
                  <td style={{ padding: '6px 4px' }}>
                    {puedeCalificar ? (
                      <select
                        className="input-field"
                  ...
                  <td style={{ padding: '6px 4px' }}>
                    {p.costoBase != null ? (
                      <span style={{ fontSize: 12 }}>
                        {p.descuentoImporte ? (
                          <>
                            <span style={{ textDecoration: 'line-through', color: 'var(--muted)' }}>Gs. {p.costoBase.toLocaleString('es-PY')}</span>
                            {' '}
                            <span style={{ color: 'var(--success)' }}>Gs. {(p.costoFinal ?? 0).toLocaleString('es-PY')}</span>
                            <span style={{ color: 'var(--muted)' }}> (Socio Protector)</span>
                          </>
                        ) : (
      ...
      {puedeRegistrarAsistencia && (
        <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {mostrarFormSesion && (
            <form onSubmit={crearSesionForm} style={{ display: 'flex', flexDirection: 'column', gap: 10, borderTop: '1px solid var(--line)', paddingTop: 10 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', gap: 10 }}>
                <div>
                  <label htmlFor="nombre-opcional" style={{ fontSize: 12, display: 'block', marginBottom: 4 }}>Nombre (opcional)</label>
                  <input id="nombre-opcional" className="input-field" value={sesionNombre} onChange={(e) => setSesionNombre(e.target.value)} placeholder={actividad.nombre} />
                </div>
                <div>
                  <label htmlFor="inicio" style={{ fontSize: 12, display: 'block', marginBottom: 4 }}>Inicio</label>
                  <input id="inicio" className="input-field" type="datetime-local" value={sesionInicio} onChange={(e) => setSesionInicio(e.target.value)} required />
                </div>
                <div>
                  <label htmlFor="fin" style={{ fontSize: 12, display: 'block', marginBottom: 4 }}>Fin</label>
                  <input id="fin" className="input-field" type="datetime-local" value={sesionFin} onChange={(e) => setSesionFin(e.target.value)} required />
                </div>
                <div>
                  <label style={{ fontSize: 12, display: 'block', marginBottom: 4 }}>Tipo de evento</label>
                  <ComboBuscable ariaLabel="Tipo de evento" opciones={opcionesTipoEvento} value={sesionTipoEventoId} onChange={setSesionTipoEventoId} ningunaLabel="Sin definir" />
                </div>
              </div>
          ...
          {sesiones && sesiones.length === 0 && <p style={{ color: 'var(--muted)', fontSize: 13 }}>Sin sesiones registradas.</p>}
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead>
                <tr style={{ textAlign: 'left', borderBottom: '1px solid var(--line)' }}>
                  ...
                  <th scope="col" style={{ padding: '6px 4px' }}>Estado</th>
                  <th scope="col" style={{ padding: '6px 4px' }}>Asistencia</th>
                </tr>
              </thead>
              <tbody>
                {sesiones.map((s) => (
                  <tr key={s.id} style={{ borderBottom: '1px solid var(--line-soft)' }}>
                    ...
                    <td style={{ padding: '6px 4px', color: 'var(--muted)' }}>{new Date(s.fechaInicio).toLocaleString('es-PY')}</td>
                    <td style={{ padding: '6px 4px', color: 'var(--muted)' }}>{new Date(s.fechaFin).toLocaleString('es-PY')}</td>
                    <td style={{ padding: '6px 4px' }}>
                      <span className="badge">{s.estado}</span>
                    </td>
                    <td style={{ padding: '6px 4px' }}>
                      <Link href={`/dashboard/asistencia/eventos/${s.id}`} style={{ color: 'var(--signal)', fontSize: 12 }}>
      ...
      {puedeCalificar && (
        <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {mostrarFormEvaluacion && (
            <form onSubmit={crearEvaluacionForm} style={{ display: 'flex', flexDirection: 'column', gap: 10, borderTop: '1px solid var(--line)', paddingTop: 10 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr 1fr', gap: 10 }}>
                <div>
                  <label style={{ fontSize: 12, display: 'block', marginBottom: 4 }}>Tipo de evaluación</label>
                  <ComboBuscable ariaLabel="Tipo de evaluación" opciones={tiposEvaluacion.map((t) => ({ value: t.id, label: t.nombre }))} value={evalTipoId} onChange={setEvalTipoId} />
                </div>
                <div>
                  <label htmlFor="titulo-opcional" style={{ fontSize: 12, display: 'block', marginBottom: 4 }}>Título (opcional)</label>
                  <input id="titulo-opcional" className="input-field" value={evalTitulo} onChange={(e) => setEvalTitulo(e.target.value)} />
                </div>
                <div>
                  <label htmlFor="fecha" style={{ fontSize: 12, display: 'block', marginBottom: 4 }}>Fecha</label>
                  <input id="fecha" className="input-field" type="date" value={evalFecha} onChange={(e) => setEvalFecha(e.target.value)} />
                </div>
              </div>
          ...
          {evaluaciones && evaluaciones.length === 0 && <p style={{ color: 'var(--muted)', fontSize: 13 }}>Sin evaluaciones registradas.</p>}
          {evaluaciones && evaluaciones.length > 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {evaluaciones.map((ev) => (
                <div key={ev.id} style={{ border: '1px solid var(--line)', borderRadius: 6, padding: 10 }}>
                  <div
                  ...
                  >
                    <div>
                      ...
                      {ev.fecha && <span style={{ marginLeft: 8, fontSize: 12, color: 'var(--muted)' }}>{ev.fecha}</span>}
                    </div>
                    <span style={{ fontSize: 12, color: 'var(--signal)' }}>{evaluacionExpandida === ev.id ? 'Ocultar notas ▲' : 'Ver/cargar notas ▼'}</span>
                  </div>

                  {evaluacionExpandida === ev.id && (
                    <div style={{ marginTop: 10, borderTop: '1px solid var(--line-soft)', paddingTop: 10 }}>
                      {cargandoNotas && <Cargando texto="Cargando notas…" />}
                      {!cargandoNotas && notas && notas.length === 0 && (
                        <p style={{ color: 'var(--muted)', fontSize: 12 }}>No hay participantes inscritos en esta actividad.</p>
                      )}
                      {!cargandoNotas && notas && notas.length > 0 && (
                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                          <thead>
                            <tr style={{ textAlign: 'left', borderBottom: '1px solid var(--line)' }}>
                              <th scope="col" style={{ padding: '4px' }}>Participante</th>
                              <th scope="col" style={{ padding: '4px' }}>Calificación</th>
                              <th scope="col" style={{ padding: '4px' }}>Resultado</th>
                            </tr>
                          </thead>
                          <tbody>
                            {notas.map((n) => (
                              <tr key={n.inscripcionId} style={{ borderBottom: '1px solid var(--line-soft)' }}>
