...
export default function GuardiasPage() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <form onSubmit={crear} className="card" style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 10 }}>
            <div>
              ...
              <ComboBuscable ariaLabel="Grupo de guardia (opcional)"
      ...
      {guardias && guardias.length === 0 && <p style={{ color: 'var(--muted)', fontSize: 13 }}>No hay guardias registradas.</p>}
            ...
            {guardias.map((g) => (
              ...
              >
                ...
                <td style={{ padding: '6px 4px' }}>
                  <span
                    className="badge"
                    style={{ background: g.estado === 'FINALIZADA' ? 'var(--ok-fill)' : g.estado === 'CANCELADA' ? 'var(--bad-fill)' : 'var(--neutral-fill)' }}
                  >
                    {g.estado}
                  </span>
                </td>
