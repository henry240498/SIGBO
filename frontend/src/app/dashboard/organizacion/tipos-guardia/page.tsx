    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <Link href="/dashboard/guardias/planificacion" className="btn-primary" style={{ textDecoration: 'none', background: '#1d4ed8' }}>
      ...
      {mostrarForm && (
        <form className="card" onSubmit={guardar} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr 1fr 1fr', gap: 10 }}>
            ...
            <div>
              <label htmlFor="duracion-horas" style={{ fontSize: 12, display: 'block', marginBottom: 4 }}>Duración (horas)</label>
              <input id="duracion-horas"
                className="input-field"
                type="number"
                min={0}
                value={duracionHoras}
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          <thead>
            <tr style={{ textAlign: 'left', borderBottom: '1px solid var(--line)' }}>
              ...
              <th scope="col" style={{ padding: '6px 4px' }}>Duración (h)</th>
          ...
          <tbody>
            {tiposGuardia.map((tg) => (
              <tr key={tg.id} style={{ borderBottom: '1px solid var(--line-soft)' }}>
                <td style={{ padding: '6px 4px' }}>{tg.codigo}</td>
                <td style={{ padding: '6px 4px' }}>{tg.nombre}</td>
                <td style={{ padding: '6px 4px', color: 'var(--muted)' }}>{tg.duracionHoras ?? '-'}</td>
                <td style={{ padding: '6px 4px' }}>
                  <span className="badge" style={{ background: tg.estado === 'ACTIVO' ? 'var(--ok-fill)' : 'var(--bad-fill)' }}>
                    {tg.estado}
                  </span>
                </td>
