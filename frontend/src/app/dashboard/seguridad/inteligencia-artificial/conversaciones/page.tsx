  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50 }}>
      <div className="card" style={{ width: 640, maxHeight: '85vh', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        {!datos && !error && <Cargando texto="Cargando…" />}
        {datos && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {datos.mensajes.map((m) => (
              <div key={m.id} style={{ fontSize: 13 }}>
                <span className="badge" style={{ background: m.rol === 'USUARIO' ? 'var(--info-fill)' : 'var(--neutral-fill)', marginRight: 6 }}>{m.rol}</span>
                <span style={{ color: m.resultado === 'ERROR' ? 'var(--danger)' : 'var(--ink)' }}>{m.contenido}</span>
                {m.duracionMs != null && <span style={{ color: 'var(--muted)', fontSize: 11 }}> ({m.duracionMs}ms)</span>}
              </div>
            ))}
          </div>
        )}
        {ejecuciones && ejecuciones.length > 0 && (
          <div style={{ borderTop: '1px solid var(--line)', paddingTop: 10 }}>
            <p style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 6 }}>Herramientas usadas</p>
            {ejecuciones.map((e) => (
              <div key={e.id} style={{ fontSize: 12, padding: '4px 0', color: 'var(--muted)' }}>
                <span className="badge" style={{ background: e.resultado === 'PERMITIDO' ? 'var(--ok-fill)' : e.resultado === 'DENEGADO' ? 'var(--bad-fill)' : 'var(--warn-fill)', marginRight: 6 }}>{e.resultado}</span>
...
export default function ConversacionesIaPage() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      {conversaciones && conversaciones.length === 0 && <p style={{ color: 'var(--muted)', fontSize: 13 }}>Sin conversaciones registradas.</p>}
      {conversaciones && conversaciones.length > 0 && (
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          <thead>
            <tr style={{ textAlign: 'left', borderBottom: '1px solid var(--line)' }}>
              {puedeEliminar && (
                <th scope="col" style={{ padding: '6px 4px', width: 28 }}>
                  <input type="checkbox" checked={todasSeleccionadas} onChange={alternarTodas} />
                </th>
              )}
              <th scope="col" style={{ padding: '6px 4px' }}>Título</th>
              <th scope="col" style={{ padding: '6px 4px' }}>Usuario</th>
              <th scope="col" style={{ padding: '6px 4px' }}>Última actividad</th>
              <th scope="col" style={{ padding: '6px 4px' }}>Estado</th>
              {puedeEliminar && <th scope="col" style={{ padding: '6px 4px' }}></th>}
            </tr>
          </thead>
          <tbody>
            {conversaciones.map((c) => (
              <tr key={c.id} style={{ borderBottom: '1px solid var(--line-soft)' }}>
                ...
                <td style={{ padding: '6px 4px', color: 'var(--signal)', cursor: 'pointer' }} onClick={() => setDetalleId(c.id)}>
                  {c.titulo ?? '(sin título)'}
                </td>
                ...
                <td style={{ padding: '6px 4px' }}>
                  <span className="badge" style={{ background: c.estado === 'ACTIVA' ? 'var(--ok-fill)' : 'var(--neutral-fill)' }}>{c.estado}</span>
                </td>
                {puedeEliminar && (
                  <td style={{ padding: '6px 4px' }}>
