    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div className="card" style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
        <input
          ...
          style={{ maxWidth: 260 }}
          placeholder="Buscar por código, alias, tipo, marca o patente..."
          value={q}
      ...
      {mostrarForm && (
        <form className="card" onSubmit={crear} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr 1fr 1fr', gap: 10 }}>
            ...
            <div>
              <label htmlFor="alias" style={{ fontSize: 12, display: 'block', marginBottom: 4 }}>Alias</label>
              <input id="alias" className="input-field" value={alias} onChange={(e) => setAlias(e.target.value)} placeholder="Nombre informal (opcional)" />
            </div>
            <div>
            ...
            {filtrados.map((v) => (
              <tr key={v.id} style={{ borderBottom: '1px solid var(--line-soft)' }}>
                <td style={{ padding: '6px 4px' }}>
                  ...
                  {v.alias && <span style={{ color: 'var(--muted)' }}> ({v.alias})</span>}
                </td>
