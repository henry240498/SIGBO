...
export default function BajasDepositoPage() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <form onSubmit={registrar} className="card" style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr 1fr', gap: 10 }}>
            <div>
              ...
              <ComboBuscable opciones={opcionesTipoElemento} value={tipoElemento} onChange={(v) => setTipoElemento(v as 'EQUIPO' | 'ARTICULO')} ningunaLabel="-- seleccionar --" />
            </div>
            {tipoElemento === 'EQUIPO' ? (
              <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
            <div>
              ...
              <ComboBuscable opciones={opcionesMotivo} value={motivoBajaId} onChange={setMotivoBajaId} ningunaLabel="-- seleccionar --" />
            </div>
            <div>
      ...
      {bajas && bajas.length === 0 && <p style={{ color: '#94a3b8', fontSize: 13 }}>No hay bajas registradas.</p>}
          ...
          <tbody>
            {bajas.map((b) => (
              <tr key={b.id} style={{ borderBottom: '1px solid #1f2937' }}>
                ...
                <td style={{ padding: '6px 4px' }}>
                  <span className="badge" style={{ background: '#7f1d1d', color: '#f87171' }}>
