...
const DEPENDENCIAS_BOMBERO: Array<{ etiqueta: string; sql: string }> = [
  ...
  { etiqueta: 'marcaciones de asistencia', sql: 'SELECT COUNT(*) AS c FROM operaciones.marcaciones_asistencia WHERE bombero_id = @0' },
  { etiqueta: 'participacion en eventos de asistencia', sql: 'SELECT COUNT(*) AS c FROM operaciones.participantes_evento WHERE bombero_id = @0' },
  { etiqueta: 'asignaciones de guardia', sql: 'SELECT COUNT(*) AS c FROM operaciones.asignacion_guardias WHERE bombero_id = @0' },
  { etiqueta: 'miembro de grupo de guardia', sql: 'SELECT COUNT(*) AS c FROM operaciones.grupos_guardia_miembros WHERE bombero_id = @0' },
  { etiqueta: 'oficial a cargo de grupo de guardia', sql: 'SELECT COUNT(*) AS c FROM operaciones.grupos_guardia WHERE oficial_a_cargo_id = @0' },
