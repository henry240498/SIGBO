...
=======
14 módulos declarados en `frontend/src/lib/modulos.ts`, de los cuales **8 están
habilitados**. Un módulo se muestra si tiene `disponible: true` **y** el usuario tiene
algún permiso con su prefijo — ver [[rule--modulo-visible-por-prefijo]].

## Advertencia de nombres: `operaciones` ≠ `asistencia`

El punto donde más se confunde el vocabulario del proyecto:

| Capa | Nombre |
|---|---|
| Módulo NestJS | `operaciones` |
| Esquema SQL | `operaciones` |
| Slug de frontend | `asistencia` |
| Prefijo de permisos | `asistencia:` |

**No existe** el prefijo `operaciones:`. Al buscar permisos de asistencia o guardias, el
prefijo es `asistencia:`.

## Los módulos

### Seguridad — `disponible: true` · 85 nodos · 13 permisos
...
El corazón del módulo es `PolicyEngineService` — ver [[rule--permisos-efectivos]] y
[[decision--permisos-dinamicos]].

### Organización Institucional — `disponible: true` · 72 nodos · 53 permisos

El organigrama y todos sus catálogos. Es el módulo con más permisos (53) porque cada
catálogo tiene sus cuatro operaciones.
>>>>>>> Stashed changes
...
=======
`organizacion.parametros` es uno de los tres mecanismos de parametrización del sistema
— ver [[decision--tolerancias-parametrizables]].

### Personal — `disponible: true` · 60 nodos · 14 permisos

El expediente de cada bombero. 18 tablas, el esquema más grande.
>>>>>>> Stashed changes
...
=======
Tablas sin backend todavía: `licencias`, `historial_medico`,
`historial_disciplinario`.

### Asistencia — `disponible: true` · 50 nodos · 13 permisos

Presencia del personal: eventos, guardias, marcaciones e importación del marcador
biométrico.

Entidades: `EventoAsistencia`, `ParticipanteEvento`, `ParticipanteExterno`,
`MarcacionAsistencia`, `ToleranciaAsistencia`, `Guardia`, `AsignacionGuardia`,
`CambioGuardia`, `ImportacionMarcador`, `ImportacionMarcadorFila`.
>>>>>>> Stashed changes
...
=======
### Servicios — `disponible: true` · 20 nodos · 5 permisos · **desarrollo activo**
>>>>>>> Stashed changes
...
=======
`EstadoServicio` = `REGISTRADO` | `DESPACHADO` | `EN_CURSO` | `FINALIZADO` |
`CANCELADO`. `GravedadServicio` = `LEVE` | `MODERADA` | `GRAVE` | `CRITICA`.
>>>>>>> Stashed changes
...
=======
### Vehículos — `disponible: true` · 10 nodos · 3 permisos · sin pantalla

`Vehiculo` (esquema `vehiculos`) y `VehiculoAutorizado` (esquema `personal` — quién
puede conducir qué). `EstadoVehiculo` = `OPERATIVO` | `EN_MANTENIMIENTO` |
`FUERA_SERVICIO` | `BAJA`.

Tablas sin backend: `mantenimientos_vehiculos`, `consumos_combustible`.

### Equipos — `disponible: true` · 15 nodos · 4 permisos · sin pantalla

`Equipo`, `CategoriaEquipo`, `PrestamoEquipo`. `EstadoEquipo` = `OPERATIVO` |
`EN_MANTENIMIENTO` | `DANIADO` | `BAJA` | `PRESTADO`. `EstadoPrestamoEquipo` =
`PRESTADO` | `DEVUELTO` | `EXTRAVIADO` | `DANIADO`.

Tabla sin backend: `mantenimientos_equipos`.

### Publicaciones — `disponible: true` · 6 nodos · 1 permiso

Contenido institucional público. Único módulo con endpoints **sin autenticación**
(`GET /publicaciones/publicas`, `/estadisticas`). Persiste en el esquema `contenido`
(migración 023). Administrar exige `publicaciones:administrar` **o**
>>>>>>> Stashed changes
...
<<<<<<< Updated upstream
=======
| **Guardias** | usa `operaciones` | Ya funciona **dentro de Asistencia** con permisos `asistencia:guardias_*`. El slug propio está sin usar |
>>>>>>> Stashed changes
...
=======
crear la carpeta de rutas. Sin el paso del medio no aparece para nadie.
>>>>>>> Stashed changes
...
=======
node .context/graph/context.mjs --dominio servicios --tipo RULE
```
>>>>>>> Stashed changes
