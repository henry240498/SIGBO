...
=======
esa sección es la más importante, porque explica qué no vas a poder hacer fácil y por
qué el sistema no está "mal hecho" cuando te choques con el límite.

| # | Decisión | Costo principal |
|---|---|---|
| 1 | [[decision--monolito-modular]] — monolito NestJS, no microservicios | Sin event sourcing ni búsqueda full-text; escala solo vertical |
| 2 | [[decision--sqlserver-en-vez-de-postgres]] — SQL Server Express | Sin `JSONB`; límite de 10 GB; sin TimescaleDB |
>>>>>>> Stashed changes
...
=======
| 8 | [[decision--tolerancias-parametrizables]] — reglas de asistencia como datos | Tres mecanismos de parametrización coexisten |
>>>>>>> Stashed changes
...
tiempo de ejecución.
=======
`export type` de estados **no lo permite en la base**. Las dos cosas compilan y fallan
...
a propósito.
=======
### Sin librería de UI (#3)

No hay tokens ni variables: los hex se repiten literalmente. Inventar un color
"parecido" produce deriva visual que nadie detecta hasta que se ven dos pantallas juntas.
Ver [[rule--sin-clases-css-nuevas]].
>>>>>>> Stashed changes
...
=======
1. **Tema claro a medio camino.** `configuracion.registry.ts` define
   `tokens.background: '#f3f7f8'` y `appearance.theme: 'auto'`, pero las ~47 pantallas
   tienen el oscuro hardcodeado. Hay que elegir un lado antes de tocar apariencia —
   [[rule--tema-oscuro-fijo]].

2. **Tres mecanismos de parametrización.** `organizacion.parametros`,
   `operaciones.tolerancias_asistencia` y el registro de configuración
   (`configuracion_valores` + `configuracion_versiones`). Cada uno tiene su razón;
   al agregar un parámetro, elegir el que corresponde en vez de crear un cuarto.

3. **`ON DELETE CASCADE` sobre documentos formales.** Una comunicación `FINALIZADA` se
   borra si alguien borra el servicio. Si el registro tiene valor legal, la FK debería
   ser `NO ACTION` y obligar a anular primero — [[rule--una-comunicacion-por-servicio]].

4. **Sin caché de permisos.** Correcto hoy (los cambios surten efecto de inmediato) y
   costoso si crece el uso. Ponerle caché obliga a invalidar en cinco puntos distintos —
   [[rule--permisos-efectivos]].

5. **Croquis embebido en base64.** Cada guardado retransmite la imagen completa y engorda
   la fila. La solución estructural es subirlo a `/uploads` y guardar la referencia; la
   infraestructura ya existe (`multer`) — [[error--413-croquis-grande]].
...
=======
Después: `node .context/graph/build-graph.mjs`.
>>>>>>> Stashed changes
