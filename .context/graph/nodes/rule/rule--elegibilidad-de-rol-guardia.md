...
terminos: [elegibilidad, requisito, rol, guardia, titular, chofer, autorizado, cargo, rango, tipo, configura, tablas, entre, filas, and, columnas, bombero, califica, coincide, alguna, fila, activa, requisitos, cumpliendo, todas, nulas, esa, configuradas, restringe]
---

# La elegibilidad para un rol de guardia se configura en tablas, con OR entre filas y AND entre columnas

Un bombero califica para un rol si coincide con ALGUNA fila activa de requisitos_rol_guardia, cumpliendo TODAS las columnas no nulas de esa fila. Sin filas configuradas, no se restringe.
...
- `affects` → [[entity--requisito-rol-guardia|RequisitoRolGuardia]]
- `affects` → [[service--guardias-elegibilidad|ElegibilidadService]]
- `affects` → [[entity--vehiculo-autorizado|VehiculoAutorizado]]
- `belongs_to` → [[domain--guardias|Guardias]]

## Referenciado por

- [[workflow--guardia-y-pernocte|Operacion de una guardia: grupos, asignaciones, presencia, novedades y pernoctes]] `contains` →
