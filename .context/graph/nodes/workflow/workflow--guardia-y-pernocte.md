...
terminos: [guardia, grupo, asignacion, presencia, horario, novedad, inspeccion, pernocte, cumplimiento, turno, operacion, grupos, asignaciones, novedades, pernoctes, arma, desde, roles, titular, chofer, asigna, personal, validando, elegibilidad, registran, horarios, quedan, inspecciones, parte]
---

# Operacion de una guardia: grupos, asignaciones, presencia, novedades y pernoctes

Una guardia se arma desde grupos con roles TITULAR/CHOFER, se asigna personal validando elegibilidad, se registran horarios y presencia, y quedan novedades, inspecciones y pernoctes como parte del turno.
...
- `contains` → [[rule--elegibilidad-de-rol-guardia|La elegibilidad para un rol de guardia se configura en tablas, con OR entre filas y AND entre columnas]]
- `contains` → [[rule--guardias-vive-en-operaciones|Guardias es un modulo propio cuyas tablas viven en el esquema operaciones]]
- `affects` → [[entity--guardia|Guardia]]
- `affects` → [[entity--grupo-guardia|GrupoGuardia]]
- `affects` → [[entity--asignacion-guardia|AsignacionGuardia]]
- `affects` → [[entity--pernocte|Pernocte]]
- `affects` → [[api--guardias-guardias|GuardiasController]]
