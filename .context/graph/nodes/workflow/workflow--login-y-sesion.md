...
terminos: [login, sesion, token, refresh, jwt, bcrypt, 401, logout, localstorage, refresco, credenciales, validadas, persistida, seguridad, sesiones, access, corto, largo, dispara, reintento, automatico, cliente]
---

# Login, sesion y refresco de token

Credenciales validadas con bcrypt, sesion persistida en seguridad.sesiones, access token corto y refresh token largo. El 401 dispara un reintento automatico en el cliente.
...
- `contains` → [[rule--bloqueo-tras-cinco-intentos|Cinco intentos fallidos bloquean la cuenta 15 minutos]]
- `contains` → [[rule--permisos-efectivos|El permiso efectivo es roles vigentes mas directos concedidos menos directos denegados]]
- `affects` → [[entity--sesion|Sesion]]
- `affects` → [[entity--usuario|Usuario]]
- `belongs_to` → [[domain--seguridad|Seguridad]]

## Referenciado por

- [[rule--bloqueo-tras-cinco-intentos|Cinco intentos fallidos bloquean la cuenta 15 minutos]] `contains` →
