---
id: domain--organizacion
tipo: DOMAIN
nombre: Organización Institucional
nivel: L0
dominio: organizacion
estado: ACTIVO
resumen: "Modulo funcional \"Organización Institucional\". Habilitado en la navegacion."
archivos:
  - frontend/src/lib/modulos.ts
terminos: [organizacion, institucional]
---

# Organización Institucional

Modulo funcional "Organización Institucional". Habilitado en la navegacion.


## Archivos

- `frontend/src/lib/modulos.ts`

## Referenciado por

- [[entity--ascenso|Ascenso]] `belongs_to` →
- [[entity--brigada|Brigada]] `belongs_to` →
- [[entity--cargo|Cargo]] `belongs_to` →
- [[entity--compania|Compania]] `belongs_to` →
- [[entity--cuartel|Cuartel]] `belongs_to` →
- [[entity--departamento|Departamento]] `belongs_to` →
- [[entity--designacion|Designacion]] `belongs_to` →
- [[entity--especialidad|Especialidad]] `belongs_to` →
- [[entity--feriado|Feriado]] `belongs_to` →
- [[entity--identidad-institucional|IdentidadInstitucional]] `belongs_to` →
- [[entity--parametro|Parametro]] `belongs_to` →
- [[entity--rango|Rango]] `belongs_to` →
- [[entity--tipo-guardia|TipoGuardia]] `belongs_to` →
- [[entity--turno|Turno]] `belongs_to` →
- [[entity--unidad|Unidad]] `belongs_to` →
- [[table--organizacion-rangos|organizacion.rangos]] `belongs_to` →
- [[table--organizacion-cargos|organizacion.cargos]] `belongs_to` →
- [[table--organizacion-especialidades|organizacion.especialidades]] `belongs_to` →
- [[table--organizacion-companias|organizacion.companias]] `belongs_to` →
- [[table--organizacion-cuarteles|organizacion.cuarteles]] `belongs_to` →
- [[table--organizacion-brigadas|organizacion.brigadas]] `belongs_to` →
- [[table--organizacion-departamentos|organizacion.departamentos]] `belongs_to` →
- [[table--organizacion-unidades|organizacion.unidades]] `belongs_to` →
- [[table--organizacion-turnos|organizacion.turnos]] `belongs_to` →
- [[table--organizacion-tipos-guardia|organizacion.tipos_guardia]] `belongs_to` →
- [[table--organizacion-designaciones|organizacion.designaciones]] `belongs_to` →
- [[table--organizacion-ascensos|organizacion.ascensos]] `belongs_to` →
- [[table--organizacion-parametros|organizacion.parametros]] `belongs_to` →
- [[table--organizacion-feriados|organizacion.feriados]] `belongs_to` →
- [[table--organizacion-identidad-institucional|organizacion.identidad_institucional]] `belongs_to` →
- [[component--modulo-organizacion|organizacion (modulo NestJS)]] `belongs_to` →
- [[service--organizacion-ascensos|AscensosService]] `belongs_to` →
- [[service--organizacion-brigadas|BrigadasService]] `belongs_to` →
- [[service--organizacion-cargos|CargosService]] `belongs_to` →
- [[service--organizacion-companias|CompaniasService]] `belongs_to` →
- [[service--organizacion-cuarteles|CuartelsService]] `belongs_to` →
- [[service--organizacion-dashboard|DashboardService]] `belongs_to` →
- [[service--organizacion-departamentos|DepartamentosService]] `belongs_to` →
- [[service--organizacion-designaciones|DesignacionesService]] `belongs_to` →
- [[service--organizacion-especialidades|EspecialidadesService]] `belongs_to` →
- [[service--organizacion-feriados|FeriadosService]] `belongs_to` →
- [[service--organizacion-identidad-institucional|IdentidadInstitucionalService]] `belongs_to` →
- [[service--organizacion-parametros|ParametrosService]] `belongs_to` →
- [[service--organizacion-rangos|RangosService]] `belongs_to` →
- [[service--organizacion-tipos-guardia|TiposGuardiaService]] `belongs_to` →
- [[service--organizacion-turnos|TurnosService]] `belongs_to` →
- [[service--organizacion-unidades|UnidadesService]] `belongs_to` →
- [[api--organizacion-ascensos|AscensosController]] `belongs_to` →
- [[api--organizacion-brigadas|BrigadasController]] `belongs_to` →
- [[api--organizacion-cargos|CargosController]] `belongs_to` →
- [[api--organizacion-companias|CompaniasController]] `belongs_to` →
- [[api--organizacion-cuarteles|CuartelsController]] `belongs_to` →
- [[api--organizacion-dashboard|DashboardController]] `belongs_to` →
- [[api--organizacion-departamentos|DepartamentosController]] `belongs_to` →
- [[api--organizacion-designaciones|DesignacionesController]] `belongs_to` →
- [[api--organizacion-especialidades|EspecialidadesController]] `belongs_to` →
- [[api--organizacion-feriados|FeriadosController]] `belongs_to` →
- [[api--organizacion-identidad-institucional|IdentidadInstitucionalController]] `belongs_to` →
- [[api--organizacion-parametros|ParametrosController]] `belongs_to` →
- [[api--organizacion-rangos|RangosController]] `belongs_to` →

---
<sub>Nodo derivado — generado por `build-graph.mjs`, no editar a mano.</sub>
