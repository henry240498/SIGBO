...
terminos: [sqlserver, express, sqlexpress, mdf, ldf, base, datos, motor, 1433, dbeaver, sql, server, 2019, instancia, local, login, sigbo, archivos, raiz, repositorio]
---

# SQL Server 2019 Express (instancia SQLEXPRESS local)

Motor de base de datos. Instancia local SQLEXPRESS, login SQL sigbo_app, archivos MDF/LDF en la raiz del repositorio.
...
- [[decision--sqlserver-en-vez-de-postgres|SQL Server 2019 Express en vez de PostgreSQL + TimescaleDB]] `depends_on` →
- [[error--pool-conexion-colgada|Timeouts de exactamente 15 segundos en todos los endpoints]] `originates_from` →
- [[error--quoted-identifier-en-migraciones|Las migraciones fallan sin QUOTED_IDENTIFIER ON]] `originates_from` →
